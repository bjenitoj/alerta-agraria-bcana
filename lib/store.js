...
export function sharedState() {
  ...
  return {
    hidden: (store.dismissedIds || []).filter((key) => String(key).startsWith("u:") || String(key).startsWith("t:")),
...
export function deleteAllArchived() {
  const store = ensureArchive(loadRaw());
  for (const entry of store.archive) dismiss(store, entry);
  store.archive = [];
  save(store);
  return store;
}
