...
import { classify, decorateItem, isAgrario, isCastillaLeon, isPesca, isAfterMadridDay, parseDate, topicOf } from "./classify.js";
...
function recentEnough(iso) {
  ...
  const now = Date.now();
  if (isAfterMadridDay(t)) return false;
...
function toItem(sourceId, sourceName, title, url, summary, publishedAt, extra = "") {
  ...
  const when = parseDate(publishedAt, blob) || new Date().toISOString();
  return decorateItem({
...
async function collectFega() {
  ...
  $(".card-item, .views-row").each((_, el) => {
    ...
    const publishedAt = parseDate(
      node.find("time").attr("datetime") || node.find("time").text() || node.find(".datetime").text()
    );
    if (!recentEnough(publishedAt)) return;
    out.push(
...
async function collectMapaPrensa() {
  ...
  return uniqueByUrl(out).slice(0, 25);
}
...
async function collectPacCyl() {
  ...
  for (const page of urls) {
    ...
    $("a").each((_, el) => {
      ...
      out.push(
        toItem(sourceId, sourceName, title, absUrl(href, page), `Publicado en ${page}`, null, title)
      );
    });
  }
  return uniqueByUrl(out).slice(0, 40);
}
...
async function collectJovenesCyl() {
  ...
  return uniqueByUrl(out).slice(0, 30);
}
...
async function collectItacyl() {
  ...
  $("a").each((_, el) => {
    ...
    out.push(
      toItem("itacyl", "ITACyL", title, absUrl(href, "https://www.itacyl.es/"), "Novedad del ITACyL", null, title)
    );
  });
  return uniqueByUrl(out).slice(0, 15);
}
...
export async function collectAll() {
  ...
  return { items: uniqueByUrl(all), reports };
}
